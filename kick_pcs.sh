	...
	for mit in "${MIT[@]}"; do
		for l in "${LOOKAHEAD[@]}"; do
			for t in "${THREADS[@]}"; do
				...
				run_series pcs_mmap_mv pcs.csv THREADS=$t LOOKAHEAD=$l OBJECTS=$o MIT=$mit MMAP_MV_PAGE_SIZE=256
				run_series pcs_mmap_mv pcs.csv THREADS=$t LOOKAHEAD=$l OBJECTS=$o MIT=$mit MMAP_MV_PAGE_SIZE=4096
			done
		done
	done
done
