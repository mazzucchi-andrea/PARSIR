...
set style line 1 lw 2 pt 7 ps 1.4  # grid_ckpt: filled circle
set style line 2 lw 2 pt 5 ps 1.4  # chunk_ckpt: filled square
set style line 3 lw 2 pt 9 ps 1.4  # full_ckpt: filled triangle

# Loop over objects to generate one plot per objects value
do for [m in m_values] {
    do for [l in spec_windows] {
        set title sprintf("Phold model (M = %s, Speculative Windows %s)", m, l)
        set xlabel "Threads"
        set ylabel "Throughput (events/s)"
        set output sprintf("plots/phold/throughput_obj1024_spec_windows%s_m%s.png", m, l)
...
do for [t in ta] {
    do for [l in spec_windows] {
        set title sprintf("PCS model (MIT = %s, Speculative Windows %s)", t, l)
        set xlabel "Threads"
        set ylabel "Throughput (events/s)"
    ...
    do for [l in spec_windows] {
        ...
        plot for [c in ckpt_types] 'phold_bench.csv' using \
            ...
            (strcol(COL_CKPT) eq c && \
                ...
                strcol(COL_LOOKAHEAD) eq l && \
                column(COL_EPOCHS) > 0 \
                ? (column(COL_RB) / column(COL_EPOCHS)) : 1/0) \
