...
uint64_t book_intermediate_target(uint64_t instruction_address, unsigned long instruction_size) {
    ...
    for (i = 0; i < intermediate_zones_index; i++) {
        if ((intermediate_zones[i] >= (instruction_address + instruction_size) - 128) &&
            (intermediate_zones[i] <= (instruction_address + instruction_size) + 127) && (intermediate_flags[i] == 0)) {
...
void build_patches(void) {
    ...
    for (i = 0; i < target_instructions; i++) {
        ...
        jmp_displacement = (int)((char *)jmp_target -
                                 ((char *)(patches[i].code) + 50)); // this is because we substitute the original call
                                                                    // instruction offset with the one observable after
                                                                    // the asm-patch instructions copy
        pos = 0;
        v[pos++] = 0xe8;
        ...
        patches[i].code = patches[i].code + save_reg_lea; // the size of the instruction
                                                          // needed to save rcx in GS and
        ...
        if (size >= 5) {
            ...
            jmp_displacement = (int)((char *)jmp_target -
                                     ((char *)(instruction_address) + 5)); // this is because we substitute the original
            ...
            if (intermediate_target == 0x0) {
                ...
                jmp_target = (char *)(patches[i].code);
                jmp_displacement = (int)((char *)jmp_target - ((char *)(intermediate_target) +
                ...
                jmp_displacement = (signed char)((char *)jmp_target -
                                                 ((char *)(instruction_address) + 2)); // this is because we substite
                                                                                       // the original instrucion with a
                                                                                       // 2-byte relative jmp
                AUDIT
        ...
        jmp_back_displacement =
            (char *)jmp_target - ((char *)(patches[i].code) + size + 5); // here we go beyond the instruction+jmp -
        ...
        if (instructions[i].rip_relative == 'y') { // in the patch we need to change the actual offset to be
                                                   // applied to RIP
            effective_operand_address = instructions[i].effective_operand_address; // this is based on the offset
                    ...
                    if (strstr(buffer, "mov") && ((strstr(buffer, ")"))) && (!(strstr(buffer, "(%rbp,"))) &&
                        ...
                        if (rip_relative == 'y') {
                            ...
                            if (temp_line[0] != '(') {
                                ...
                                p = aux = strtok(NULL, "("); // go to the next part of
