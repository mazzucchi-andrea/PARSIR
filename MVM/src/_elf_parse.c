...
void build_patches(void) {
    ...
    int test_code_size = 0x52; // this is taken from the compiled version of the src/_asm_patch.S file
    ...
    for (i = 0; i < target_instructions; i++) {
        ...
        if (size >= 5) {
            ...
            if (intermediate_target == 0x0) {
                printf("no intermediate target available for the mov instruction at runtime address %p index is %d\n",
                ...
                jmp_target = (char *)(patches[i].code);
                jmp_displacement =
                    (int)((char *)jmp_target - ((char *)(intermediate_target) +
                                                5)); // this is because we substite the original cli/nop instruction set
                                                     // with a 5-byte relative jmp from the intermediate area
                AUDIT
                ...
                // we now prepare the jump to the intermediate zone for this instruction - this becomes the real block
                // of instructions substituting the original mov instruction
                jmp_target = (char *)(intermediate_target);
                jmp_displacement = (signed char)((char *)jmp_target - ((char *)(instruction_address) +
        ...
        if (instructions[i].rip_relative ==
            'y') { // in the patch we need to change the actual offset to be applied to RIP
            effective_operand_address =
                instructions[i].effective_operand_address; // this is based on the offset from the original RIP
