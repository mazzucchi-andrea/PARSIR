...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        while (fgets(line, MAX_LINE, input_file) && count < run) {
            char ckpt_type[64];
            int t, obj, m_value, p_value, epochs, rollbacks, events, committed, filtered;
            double spec_window;

            sscanf(line, "%[^,],%d,%lf,%d,%d,%d,%d,%d,%d,%d,%d", ckpt_type, &t, &spec_window, &obj, &m_value, &p_value,
        ...
        fprintf(output_file, "%s,%d,%f,%d,%d,%d,%f,%f,%f,%f\n", ckpt_types[i], threads, spec, objects, m, p_shift,
